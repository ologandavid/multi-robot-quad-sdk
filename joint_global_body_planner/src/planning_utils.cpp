...
double poseDistance(const MultipleState &s1, const MultipleState &s2) {
  double p1 = (s1.rob1.pos - s2.rob1.pos).norm();
  double p2 = (s1.rob2.pos - s2.rob2.pos).norm();
  return p1+p2;
}
...
double stateDistance(const MultipleState &q1, const MultipleState &q2) {
  double w_pos = 1;
  double w_vel = 1;
  double rob1_dist = ((q2.rob1.pos - q1.rob1.pos).norm() * w_pos + (q2.rob1.vel - q1.rob1.vel).norm() * w_vel);
  double rob2_dist = ((q2.rob2.pos - q1.rob2.pos).norm() * w_pos + (q2.rob2.vel - q1.rob2.vel).norm() * w_vel);
  return rob1_dist + rob2_dist;
}
