...
MultipleState GraphClass::getVertex(int idx) { return vertices[idx]; }

int GraphClass::getNumVertices() { return vertices.size(); }

void GraphClass::addVertex(int idx, MultipleState q) {
  vertices[idx] = q;
  return;
}
...
void GraphClass::addAction(int idx, MultipleAction a) { actions[idx] = a; }

MultipleAction GraphClass::getAction(int idx) { return actions[idx]; }

void GraphClass::printVertex(State vertex) { printState(vertex); }
/*
...
void GraphClass::init(MultipleState q) {
  int q_init = 0;
  addVertex(q_init, q);
  g_values[q_init] = 0;
  return;
