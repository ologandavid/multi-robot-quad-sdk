...
struct State {
  ...
  bool isApprox(const State rhs) const {
    // Z velocity is overridden by action
    return ((pos.isApprox(rhs.pos)) && (vel.head<2>().isApprox(rhs.vel.head<2>())));
  }
};
...
/**
 * @brief Define multiple robot state with multiple state
 */
struct MultipleState {
  State rob1;
  State rob2;

  bool operator==(const MultipleState rhs) const {
    return ((rob1.pos == rhs.rob1.pos) && (rob1.vel.head<2>() == rhs.rob1.vel.head<2>()) 
    && (rob2.pos == rhs.rob2.pos) && (rob2.vel.head<2>() == rhs.rob2.vel.head<2>()));
  }

  bool operator!=(const MultipleState rhs) const {
    return ((rob1.pos != rhs.rob1.pos) || (rob1.vel.head<2>() != rhs.rob1.vel.head<2>()) 
    || (rob2.pos != rhs.rob2.pos) || (rob2.vel.head<2>() !=rob2.vel.head<2>()));
  }

  bool isApprox(const MultipleState rhs) const {
    return ((rob1.pos.isApprox(rhs.rob1.pos)) && (rob1.vel.head<2>().isApprox(rhs.rob1.vel.head<2>())) 
    && rob2.pos.isApprox(rhs.rob2.pos) && (rob2.vel.head<2>().isApprox(rhs.rob2.vel.head<2>())));
  }
};
...
struct MultipleStateActionResult {
  StateActionResult rob1;
  StateActionResult rob2;
};
 ...
 */
double poseDistance(const MultipleState &s1, const MultipleState &s2);
 ...
 * distance between velocity vectors of two States
 */
double stateDistance(const MultipleState &q1, const MultipleState &q2);

double stateDistanceOne(const State &q1, const State &q2);
