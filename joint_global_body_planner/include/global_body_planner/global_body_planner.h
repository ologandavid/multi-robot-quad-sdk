...
class GlobalBodyPlanner {
 ...
 private:
   ...
   */
  void robotStateCallbackr1(const quad_msgs::RobotState::ConstPtr &msg);
  void robotStateCallbackr2(const quad_msgs::RobotState::ConstPtr &msg);
   ...
   */
  FullState setStartState(FullState robot_state_, GlobalBodyPlan current_plan_);
  ...
  ros::Subscriber r1robot_state_sub_;
  ros::Subscriber r2robot_state_sub_;

  /// Subscriber for goal state messages
  ros::Subscriber goal_state_sub_;

  /// Publisher for body plan messages
  ros::Publisher r1body_plan_pub_;
  ros::Publisher r2body_plan_pub_;
  ...
  GlobalBodyPlan r1newest_plan_;
  GlobalBodyPlan r2newest_plan_;

  /// Plan data for the plan currently being executed
  GlobalBodyPlan r1current_plan_;
  GlobalBodyPlan r2current_plan_;

  /// Starting state for planner call
  FullState r1start_state_;
  FullState r2start_state_;

  /// Goal state for planner
  FullState r1goal_state_;
  FullState r2goal_state_;

  /// Current robot state
  FullState r1robot_state_;
  FullState r2robot_state_;
  ...
  std::vector<MultipleState> state_sequence_;

  /// Sequence of discrete actions in the plan
  std::vector<MultipleAction> action_sequence_;
