...
class GBPL : public RRT {
 public:
   ...
   */
  int connect(std::vector<State> &first_robot_path, PlannerClass &T, State s, const PlannerConfig &planner_config,
   ...
   * @param[in] planner_config Configuration parameters
   */
  void postProcessPath(std::vector<State> &first_robot_plan, std::vector<State> &state_sequence,
   ...
   * @param[in] planner_config Configuration parameters
   */
  void extractPath(std::vector<State> &first_robot_path, PlannerClass &Ta, PlannerClass &Tb,
   ...
   */
  void extractClosestPath(std::vector<State> &first_robot_path, PlannerClass &Ta, const State &s_goal,
                          std::vector<State> &state_sequence,
                          std::vector<Action> &action_sequence,
                          const PlannerConfig &planner_config);
  ...
  int findPlan(const PlannerConfig &planner_config, State s_start, State s_goal,
               std::vector<State> &state_sequence,
               std::vector<Action> &action_sequence, std::vector<State> &first_rob_path, ros::Publisher &tree_pub);
