...
GlobalBodyPlanner::GlobalBodyPlanner(ros::NodeHandle nh) {
  ...
  std::string body_plan_topic, discrete_body_plan_topic, body_plan_tree_topic,
      goal_state_topic;
  std::vector<double> r1goal_state_vec = {2.5, 1.0};
  std::vector<double> r2goal_state_vec = {4, -0.5};
...
bool GlobalBodyPlanner::callPlanner() {
  ...
  double plan_time2, path_length2, path_duration2, total_solve_time2,
      total_vertices_generated2, total_path_length2, total_path_duration2,
      dist_to_goal2;
  
  int vertices_generated2;
  ...
  for (int i = 0; i < num_calls_; ++i) {
    ...
    int plan_status2 = gbpl.findPlan(planner_config_, r1start_state, r1goal_state,
                                    r1state_sequence_, r1action_sequence_, first_robot_path, tree_pub_);
    first_robot_path = interpolateStates(r1state_sequence_);
    std::cout << "Here1: " << std::endl;
    int plan_status = gbpl.findPlan(planner_config_, r2start_state, r2goal_state,
                                     r2state_sequence_ ,r2action_sequence_, first_robot_path , tree_pub_);  
    ...
    r2newest_plan_.loadPlanData(plan_status, r2start_state_, dist_to_goal,
                              r2state_sequence_, r2action_sequence_, dt_,
                              replan_start_time_, planner_config_);
    std::cout << "Here 1 in function: " << std::endl;
    r1newest_plan_.loadPlanData(plan_status, r1start_state_, dist_to_goal,
                              r1state_sequence_, r1action_sequence_, dt_,
                              replan_start_time_, planner_config_);
    ...
    if ((plan_status == VALID) &&
      ...
      is_updated = true;
               ...
               (r2current_plan_.getStatus() == UNSOLVED ||
                (poseDistance(r2state_sequence_.back(), r2goal_state) <
...
void GlobalBodyPlanner::publishCurrentPlan() {
  ...
  if (r1current_plan_.isEmpty() ||
    ...
    return;
  // Check condition 3
  if (publish_after_reset_delay_ || r1newest_plan_ == r1current_plan_ || r2newest_plan_ == r2current_plan_) {
