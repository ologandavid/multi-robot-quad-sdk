...
int GBPL::connect(std::vector<State> &first_robot_path, PlannerClass &T, State s, const PlannerConfig &planner_config,
  ...
  int connect_result =
      attemptConnect(first_robot_path, s_near, s, result, planner_config, direction);
...
void GBPL::postProcessPath(std::vector<State> &first_robot_path, std::vector<State> &state_sequence,
                           std::vector<Action> &action_sequence,
  ...
  while (s != s_goal) {
    ...
    // states left
    while ((attemptConnect(first_robot_path, s, s_next, result, planner_config, FORWARD) !=
            REACHED) &&
           (s != s_next)) {
      old_state = s_next;
      old_action = a_next;
    ...
    if (s != s_next) {
      ...
      isValidStateActionPair(first_robot_path, old_state, old_action, result, planner_config);
      path_length_ += result.length;
      s = old_state;
    }
  }
...
void GBPL::extractPath(std::vector<State> &first_robot_path, PlannerClass &Ta, PlannerClass &Tb,
  ...
  postProcessPath(first_robot_path, state_sequence, action_sequence, planner_config);
}

void GBPL::extractClosestPath(std::vector<State> &first_robot_path, PlannerClass &Ta, const State &s_goal,
                              std::vector<State> &state_sequence,
                              std::vector<Action> &action_sequence,
                              const PlannerConfig &planner_config) {
...
int GBPL::findPlan(const PlannerConfig &planner_config, State s_start,
                   ...
                   std::vector<Action> &action_sequence, std::vector<State> &first_rob_path,
                   ros::Publisher &tree_pub) {
  // Perform validity checking on start and goal states
  if (!isValidState(first_rob_path, s_start, planner_config, LEAP_STANCE)) {
  ...
  if (!isValidState(first_rob_path, s_goal, planner_config, LEAP_STANCE)) {
  ...
  while (ros::ok()) {
    ...
    if (isValidState(first_rob_path, s_rand, planner_config, LEAP_STANCE)) {
      if (extend(first_rob_path, Ta, s_rand, planner_config, FORWARD, tree_pub) != TRAPPED) {
        ...
        if (connect(first_rob_path, Tb, s_new, planner_config, FORWARD, tree_pub) == REACHED) {
          goal_found = true;
    ...
    if (isValidState(first_rob_path, s_rand, planner_config, LEAP_STANCE)) {
      if (extend(first_rob_path, Tb, s_rand, planner_config, FORWARD, tree_pub) != TRAPPED) {
        ...
        if (connect(first_rob_path, Ta, s_new, planner_config, FORWARD, tree_pub) == REACHED) {
          goal_found = true;
  ...
  if (goal_found == true) {
    extractPath(first_rob_path, Ta, Tb, state_sequence, action_sequence, planner_config);
    result = VALID;
  } else {
    extractClosestPath(first_rob_path, Ta, s_goal, state_sequence, action_sequence,
                       planner_config);
