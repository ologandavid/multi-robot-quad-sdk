...
bool refineStance(const State &s, int phase, Action &a,
  ...
  while (!(grf_valid && friction_cone_valid && final_state_valid &&
    ...
    double buffer = 3e-2;
    if (phase == LEAP_STANCE) {
      isValidState(first_robot_path, s_final, planner_config, phase, pos_f[2]);
      pos_f[2] -= buffer;
    } else {
    ...
    final_state_valid = isValidState(first_robot_path, s_final, planner_config, phase);
    midstance_state_valid = isValidState(first_robot_path, s_midstance, planner_config, phase);
...
bool isValidState(std::vector<State> &first_plan_vec, const State &s, const PlannerConfig &planner_config,
                  int phase) {
  double dummy_max_valid_z = 0;
  return isValidState(first_plan_vec, s, planner_config, phase, dummy_max_valid_z);
}
...
bool isValidState(std::vector<State> &first_plan_vec, const State &s, const PlannerConfig &planner_config,
  ...
  if (!first_plan_vec.empty()) {
    bool conflicts_with_robot1 = conflictsWithFirstRobotPath(s,conflict_threshold, first_plan_vec);
    if (conflicts_with_robot1 == true) {
      return false;
    }
  }
...
bool conflictsWithFirstRobotPath(const State &state, double conflict_threshold, std::vector<State> &first_robot_path) {
  if (first_robot_path.empty()) {
    return false;
  }
  for (auto path_state : first_robot_path) {
      if (poseDistance(state, path_state) < conflict_threshold) {
        return true;
      }
  }
  return false;
...
std::vector<State> interpolateStates(const std::vector<State>& states) {
    std::vector<State> interpolatedStates;

    if (states.size() < 2) {
        // Handle cases where there are not enough states to interpolate
        return states;
    }
    ...
    for (size_t i = 1; i < states.size(); ++i) {
        totalDistance += poseDistance(states[i - 1], states[i]);
    }

    // Distance between interpolated states
    double step = totalDistance / 999;
    double accumulatedDistance = 0.0;

    for (size_t i = 0; i < states.size() - 1; ++i) {
        State start = states[i];
        State end = states[i + 1];
        double segmentDistance = poseDistance(start, end);

        while (accumulatedDistance < segmentDistance && interpolatedStates.size() < 999) {
            double ratio = accumulatedDistance / segmentDistance;
            Eigen::Vector3d interpPos = start.pos + ratio * (end.pos - start.pos);
            // Create a new State with interpolated position
            // Velocity can be set as needed, here just copying start's velocity
            State interpolatedState;
            interpolatedState.pos = interpPos;
            interpolatedState.vel = start.vel; // or however you want to handle velocity
            interpolatedStates.push_back(interpolatedState);
    ...
    // Ensure the last state is added
    interpolatedStates.push_back(states.back());

    return interpolatedStates;
}

bool isValidStateActionPair(std::vector<State> &first_robot_path, const State &s_in, const Action &a,
  ...
  while (t <= t_s) {
    ...
    if (!isValidState(first_robot_path, s_next, planner_config, phase)) {
  ...
  if (!isValidState(first_robot_path, s_takeoff, planner_config, phase)) {
  ...
  while (t < t_f) {
    ...
    if (!isValidState(first_robot_path, s_next, planner_config, FLIGHT)) {
#ifdef DEBUG_INVALID_STATE
  ...
  if (phase != CONNECT) {
    ...
    while (t < t_s_land) {
      ...
      if (!isValidState(first_robot_path, s_next, planner_config, LAND_STANCE)) {
#ifdef DEBUG_INVALID_STATE
    ...
    if (!isValidState(first_robot_path, s_final, planner_config, LAND_STANCE)) {
#ifdef DEBUG_INVALID_STATE
