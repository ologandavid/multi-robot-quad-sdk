...
bool RRT::newConfig(std::vector<State> &first_robot_path, State s, State s_near, StateActionResult &result,
  ...
  int connect_result =
      attemptConnect(first_robot_path, s_near, s, current_result, planner_config, direction);
  ...
  for (int i = 0; i < planner_config.num_leap_samples; ++i) {
    ...
    for (int j = 0; j < planner_config.num_leap_samples; ++j) {
      bool is_valid = isValidStateActionPair(first_robot_path, s_near, a_test, current_result,
                                             planner_config);
...
int RRT::attemptConnect(std::vector<State> &first_robot_path, const State &s_existing, const State &s, double t_s,
  ...
  if (isValidAction(result.a_new, planner_config)) {
    ...
    if (isValidStateActionPair(first_robot_path, s_start, result.a_new, result, planner_config)) {
      return REACHED;
    } else {
      if (attemptConnect(first_robot_path, s_existing, result.s_new, result.t_new, result,
...
int RRT::attemptConnect(std::vector<State> &first_robot_path, const State &s_existing, const State &s,
                        StateActionResult &result,
                        const PlannerConfig &planner_config, int direction) {
  ...
  return attemptConnect(first_robot_path, s_existing, s, t_s, result, planner_config, direction);
}

int RRT::extend(std::vector<State> &first_robot_path, PlannerClass &T, const State &s,
                const PlannerConfig &planner_config, int direction,
                ros::Publisher &tree_pub) {
  ...
  if (newConfig(first_robot_path, s, s_near, result, planner_config, direction, tree_pub)) {
